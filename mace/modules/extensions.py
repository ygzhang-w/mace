...
@compile_mode("script")
class AtomicQdivMACE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        self.readouts.append(LinearReadoutBlock(hidden_irreps, o3.Irreps("1x0e")))
        ...
        for i in range(num_interactions - 1):
            ...
            if i == num_interactions - 2:
                self.readouts.append(
                    NonLinearReadoutBlock(
                        hidden_irreps_out, MLP_irreps, gate, o3.Irreps("1x0e")
                    )
                )
            else:
                self.readouts.append(
                    LinearReadoutBlock(hidden_irreps_out, o3.Irreps("1x0e"))
                )
    ...
    def forward(
        ...
        for interaction, product, readout in zip(
            ...
            node_qdiv = readout(node_feats).squeeze(-1)  # [n_nodes,]
            qdiv_contributions.append(node_qdiv)
